...
def export_data():
    try:
        ...
        print(f"URL: {client.address[0]}:{client.address[1]}")
        ...
        #Extraer as columnas de extra normalizando o JSON
        ...
        if not df.empty:
            ...
            print(f"- CSV: data/citybikes_data.csv")
            print(f"- Parquet: data/citybikes_data.parquet")
            
        else:
            print("No se encontraron datos para exportar")
            
    except Exception as e:
        print(f"Error: No se pudo conectar a MongoDB. Asegúrate de que el servidor está ejecutándose.")
